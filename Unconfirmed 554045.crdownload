...
def extract_features(signal):
    ...
    for wavelet in wavelets:
        coeffs = pywt.wavedec(signal, wavelet, mode='symmetric', level=3)
        smoothness = np.mean(np.abs(coeffs[1]))
        freq_content = np.sum(coeffs[1]**2)
        corr = np.corrcoef(signal, pywt.waverec(coeffs, wavelet))[0, 1]
        features.append([smoothness, freq_content, corr])
    return features
...
inter = np.array([extract_features(signal) for signal in Reference_genome])
X = np.hstack((f1, f2, f3))
tags = modifiedEM(X,GroundTruth}
